...
mb-proto-mesh decisions:
  - 0.1.9 (2026-05-24): `MeshFrame::StreamOpenAccepted` was appended as an L5 Control frame (msg_type 52) and `StreamOpen` carries a per-open `open_token`, so `StreamOpen` is no longer fire-and-forget and stale ACKs cannot confirm a new open. Ingress echoes the token in accepted/reject only after local bus stream re-entry succeeds/fails; egress `connect` waits for matching accepted/reject/timeout before reporting path success. The accepted variant is append-only to preserve existing bincode discriminants for older frame variants. Proven by codec roundtrip, mesh-peer egress `stream_connect_requires_open_ack` and `stream_connect_ignores_stale_open_ack_token`, ingress stream re-entry, product topology fixtures, and Run2 failover gate.
