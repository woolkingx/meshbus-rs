...
//! `BusSessionRequest` for the direct TCP path. `Verdict::Accept(SinkId)`
//! projects onto `BusSessionRequest.target_sink`; `policy.route_group` and
//! `transport.schedule_hint` are read back and stamped on the request.
//! Direct TCP has no protocol reply, so `Deny` and `Drop` both close the
//! client; the distinction is log-only.
