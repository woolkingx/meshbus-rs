...
boundary rules:
  ...
  - Raw UDP accepts stream and datagram control/data; `StreamOpen` re-enters the local bus and replies with `StreamOpenAccepted` only after local open succeeds, or `StreamOpenReject` when local open fails.
  ...
  - `StreamData` that arrives before `StreamOpen` is bounded per pending session; overflow emits `NativeDropReason::StreamDataPending` and does not grow memory without limit.

mesh-bus-ingress-mesh-peer-udp decisions:
  - 0.1.7 (2026-05-25): pending pre-open stream data is bounded (`DEFAULT_PENDING_STREAM_DATA_MAX_BYTES = 64 KiB`, builder override available). Overflow emits observable native drop reason `stream_data_pending` and keeps dispatch/session counters untouched. Proven by `stream_data_before_open_is_bounded_and_observable` and full `cargo test -p mesh-bus-ingress-mesh-peer-udp`.
  - 0.1.6 (2026-05-24): Stream re-entry now positively acknowledges open success. Ingress sends `StreamOpenAccepted` after `open_peer_stream_session` succeeds and sends `StreamOpenReject` on typed local open failure. This keeps peer health and dispatch success tied to remote bus re-entry, not local UDP enqueue. Proven by ingress stream tests, product topology fixtures, and Run2 failover gate.
